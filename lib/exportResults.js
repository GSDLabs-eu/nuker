  ...
  const codeFragment2 = function onLoad(data) {
    data.forEach((testResult, index) => {
      const chart = new Chart(document.getElementById(`chartPic${index}`).getContext('2d'), {
        type: 'bar',
        data: {
          labels: Array.from(new Array(testResult.responseTimeAxis.length), (val, i) => i + 1),
          datasets: [{
            label: 'Response Times',
            yAxisID: 'A',
            data: testResult.responseTimeAxis,
            type: 'line',
            showLine: false,
            pointRadius: 2,
        ...
        options: {
          ...
          scales: {
            yAxes: [{
              ...
              scaleLabel: {
                display: true,
                labelString: 'Response times in ms',
              },
              ticks: {
                beginAtZero: true,
              ...
              ticks: {
                beginAtZero: true,
                suggestedMax: testResult.responseTimeAxis.length,
              },
              gridLines: {
                display: false,
              },
