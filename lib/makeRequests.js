...
async function networkRequest(config, seq) {
  ...
  if (!isEmpty(config.formFiles) || !isEmpty(config.formFields)) {
    const formKey = Object.keys(config.formFiles).length ? 'formData' : 'form';
    ...
    options[formKey] = { ...attachedFormFiles, ...config.formFields };
  }
