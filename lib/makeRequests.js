...
async function networkRequest(apiUrl, requestMethod, payloadPaths, keyValuePairs, id, requestCount) {
  const options = {
  ...
  debug(`Sending request ${id}/${requestCount} to endpoint ${apiUrl}`);
  ...
  try {
    ...
    debug(`Request ${id} was rejected or timed out.`);
    return {
      failed: true,
      err,
    };
...
async function makeRequests(
  ...
  return new Promise((resolve) => {
    ...
    function completed(id, response) {
      if (response.failed) allResults.failed[id] = response;
    ...
    async function execute() {
      for (let id = 1; id <= requestCount; id += 1) {
        if (id > 1) await wait(timeOffset);
        networkRequest(apiUrl, requestMethod, payloadPaths, keyValuePairs, id, requestCount)
